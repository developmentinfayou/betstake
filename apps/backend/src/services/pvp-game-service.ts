import { PVPGame, PVPGameStatus, PVPGameType, User, Wallet, Currency } from '@casino/database';
  ...
  static async createGame(creatorId: string, gameType: PVPGameType, mode: string, betAmount: number, currency: string) {
    ...
    try {
      return await session.withTransaction(async () => {
        ...
        await WalletService.debitBalanceWithSession(creatorId, currency as Currency, betAmount, session);
  ...
  static async joinGame(gameId: string, playerId: string) {
    ...
    try {
      return await session.withTransaction(async () => {
        ...
        await WalletService.debitBalanceWithSession(playerId, game.currency as Currency, game.betAmount, session);
  ...
  static async makeMove(gameId: string, playerId: string, move: any) {
    ...
    try {
      return await session.withTransaction(async () => {
        ...
        // Check for game end
        const winner = this.checkGameEnd(game.gameState, game.gameType) as string | null;
        if (winner) {
          game.status = PVPGameStatus.FINISHED;
          game.winner = winner as any;
