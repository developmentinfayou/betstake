...
export function useGameLogic({ gameType, currency = 'USD' }: UseGameLogicProps) {
  ...
  const placeBet = async (gameParams: any) => {
    if (amount > balance) {
  ...
  const startAutoBet = async (gameParams: any, config: any) => {
    try {
