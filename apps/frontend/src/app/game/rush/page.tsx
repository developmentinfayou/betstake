...
export default function RushPage() {
  ...
  const [gameParams, setGameParams] = useState<RushGameParams>({ difficulty: 'medium', targetMultiplier: 2.0 });
  ...
  const placeBet = async () => {
    ...
    setLoading(true);
    try {
      const response = await betAPI.place({ gameType: 'RUSH', currency: 'USD', amount, gameParams });
  ...
  const handleStartAutoBet = async (config: AutoBetConfig) => {
    try {
      await betAPI.startAutobet({ gameType: 'RUSH', currency: 'USD', amount, gameParams, config });
    ...
    <div className="min-h-screen bg-gray-900">
      <header className="border-b border-gray-800">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/" className="text-2xl font-bold gradient-text">← Rush</Link>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="card">
              ...
              <RushGameControls onChange={setGameParams} disabled={loading || autoBetActive} />
            </div>
          </div>
