...
export default function WheelPage() {
  ...
  const [gameParams, setGameParams] = useState<WheelGameParams>({ risk: 'medium', segments: 10 });
  ...
  const placeBet = async () => {
    ...
    setLoading(true);
    try {
      const response = await betAPI.place({ gameType: 'WHEEL', currency: 'USD', amount, gameParams });
  ...
  const handleStartAutoBet = async (config: AutoBetConfig) => {
    try {
      await betAPI.startAutobet({ gameType: 'WHEEL', currency: 'USD', amount, gameParams, config });
    ...
    <div className="min-h-screen bg-gray-900">
      <header className="border-b border-gray-800">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/" className="text-2xl font-bold gradient-text">← Wheel</Link>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="card">
              <h2 className="text-2xl font-bold mb-6">Wheel</h2>
              ...
              {result && (
                <div className={`mb-6 p-6 rounded-lg text-center ${result.multiplier > 0 ? 'bg-green-900/20 border border-green-500' : 'bg-red-900/20 border border-red-500'}`}>
                  <div className="text-6xl font-bold mb-2" style={{ color: result.color }}>⭕</div>
                  <div className="text-2xl mb-2">Segment {result.segment}</div>
                  <div className="text-xl">{result.multiplier}x Multiplier</div>
                </div>
              )}

              <WheelGameControls onChange={setGameParams} disabled={loading || autoBetActive} />
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="card">
              ...
              {betMode === 'manual' && (
                <ManualBetControls amount={amount} balance={balance} onAmountChange={setAmount} onBet={placeBet} disabled={autoBetActive} loading={loading} multiplier={result?.multiplier || 1.5} />
              )}
              {betMode === 'auto' && (
