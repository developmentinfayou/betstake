...
export default function CrashPage() {
  ...
  const [trenballBetType, setTrenballBetType] =
    useState<TrenballBetType>("green");
  const [balance, setBalance] = useState(0);
  const [socket, setSocket] = useState<Socket | null>(null);
  const [gameState, setGameState] = useState<GameState>("waiting");
  const [currentMultiplier, setCurrentMultiplier] = useState(1.0);
  const [crashPoint, setCrashPoint] = useState<number | null>(null);
  const [trenballResult, setTrenballResult] = useState<TrenballResult | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    newSocket.on("player-cashed-out", (data) => {
      if (data.mode === gameMode) {
        setBets((prev) =>
          prev.map((b) =>
            b.odId === data.userId
              ? { ...b, cashedOut: true, cashoutAt: data.multiplier }
              : b
          )
        );
      ...
      if (data.mode === gameMode) {
        ...
        if (gameMode === "trenball" && myBet) {
          ...
          if (data.trenballResult?.type === myBetType) {
            toast.success(
              `${myBetType.toUpperCase()} wins! +$${(
                myBet.amount * data.trenballResult.multiplier
              ).toFixed(2)}`
            );
            loadBalance();
          } else {
            toast.error(
              `${data.trenballResult?.type.toUpperCase()} won. Better luck next time!`
            );
          }
        }
      }
    ...
    newSocket.on("jackpot-triggered", (data) => {
      if (data.userId === userId) {
        toast.success(
          `🎉 JACKPOT! ${data.conditionName} - Won $${data.prizeAmount}!`,
          { duration: 5000 }
        );
      }
    });
  ...
  const drawGraph = () => {
    ...
    if (gameState === "playing" || gameState === "crashed") {
      ...
      const y =
        canvas.height - (canvas.height / maxMult) * (currentMultiplier - 1);
            ...
            <div className="card">
              ...
              {gameMode === "classic" && (
                <div className="mt-4 grid grid-cols-2 gap-4">
                  ...
                  {gameState === "playing" && myBet && !myBet.cashedOut && (
                    <button
                      onClick={cashout}
                      className="btn-primary py-3 col-span-2 bg-yellow-500 hover:bg-yellow-600"
                    >
              ...
              {gameMode === "trenball" && gameState === "betting" && !myBet && (
                <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                  {(["crash", "red", "green", "moon"] as TrenballBetType[]).map(
                    (type) => (
                      <button
                        key={type}
                        onClick={() => {
                        ...
                        className={`${getTrenballButtonColor(
                          type
                        )} py-4 rounded-lg font-bold text-center`}
                      >
                        <div className="text-lg capitalize">{type}</div>
                        <div className="text-sm opacity-80">
                          {getTrenballMultiplier(type)}
                        </div>
                      </button>
                    )
                  )}
                ...
                trenballResult && (
                  <div className="mt-4 p-6 rounded-lg bg-gray-800 text-center">
                    ...
                    <div className="text-xl text-gray-400">
                      {trenballResult.multiplier}x payout
                    </div>
                  </div>
                )}
            </div>
            ...
            <div className="card">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {bets.map((bet, i) => (
                  <div
                    key={i}
                    className="flex justify-between text-sm p-2 bg-gray-800 rounded"
                  >
