              ...
              {result && (
                ...
                >
                  ...
                  <div className="text-2xl mb-2 capitalize">
                    {getNumberColor(result.number)}
                  </div>
                  <div className="text-xl">
                    {result.totalPayout > 0
                      ? `Won $${result.totalPayout.toFixed(2)}`
                      : "Lost"}
                  </div>
                  {result.winningBets.length > 0 && (
                    <div className="text-sm text-gray-400 mt-2">
                      {result.winningBets.length} winning bet(s)
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="space-y-6">
            <div className="card">
              ...
              {betMode === "manual" && (
                <ManualBetControls
                  amount={
                    gameParams.bets.reduce((sum, b) => sum + b.amount, 0) ||
                    amount
                  }
                  balance={balance}
                  onAmountChange={() => { }}
