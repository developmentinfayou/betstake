...
export default function BalloonPage() {
  ...
  const placeBet = async () => {
    ...
    setLoading(true);
    try {
      const response = await betAPI.place({ gameType: 'BALLOON', currency: 'USD', amount, gameParams });
      ...
      if (gameResult.won) {
        ...
        toast.error(`Balloon burst at ${gameResult.result.burstAt} pumps!`);
  ...
  const handleStartAutoBet = async (config: AutoBetConfig) => {
    try {
      await betAPI.startAutobet({ gameType: 'BALLOON', currency: 'USD', amount, gameParams, config });
    ...
    <div className="min-h-screen bg-gray-900">
      <header className="border-b border-gray-800">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/" className="text-2xl font-bold gradient-text">← Balloon</Link>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="card">
              <h2 className="text-2xl font-bold mb-6">Balloon</h2>
              ...
              {result && (
                <div className={`mb-6 p-6 rounded-lg text-center ${result.won ? 'bg-green-900/20 border border-green-500' : 'bg-red-900/20 border border-red-500'}`}>
                  ...
                  <div className="text-2xl mb-2">{result.won ? '🎉 WIN!' : '💥 BURST!'}</div>
                  <div className="text-xl">Pumps: {result.pumps} / Burst at: {result.burstAt}</div>
                  <div className="text-lg text-gray-400">Multiplier: {result.multiplier.toFixed(2)}x</div>
                </div>
              )}

              <BalloonGameControls onChange={setGameParams} disabled={loading || autoBetActive} />
            </div>
          </div>
