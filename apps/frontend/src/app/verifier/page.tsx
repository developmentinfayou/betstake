      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="card">
          ...
          <p className="text-gray-400 mb-6">
            Verify any bet by entering the seed data. After rotating your seed
            pair, you can use the revealed server seed to verify all previous
            bets were fair.
          </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateGameParams("gridSize", parseInt(e.target.value))
                    }
                    className="input w-full"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateGameParams("minesCount", parseInt(e.target.value))
                    }
                    className="input w-full"
                    min="1"
                    max="24"
            ...
            {gameType === "PLINKO" && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateGameParams("rows", parseInt(e.target.value))
                  }
                  className="input w-full"
                >
            ...
            {gameType === "WHEEL" && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateGameParams("segments", parseInt(e.target.value))
                  }
                  className="input w-full"
                >
            ...
            {gameType === "TOWER" && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateGameParams("floors", parseInt(e.target.value))
                  }
                  className="input w-full"
                >
                  {[8, 10, 12, 15].map((f) => (
            ...
            {gameType === "STAIRS" && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateGameParams("steps", parseInt(e.target.value))
                  }
                  className="input w-full"
                >
                  {[8, 10, 12, 15].map((s) => (
            ...
            {gameType === "BALLOON" && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateGameParams("maxPumps", parseInt(e.target.value))
                  }
                  className="input w-full"
                  min="10"
                  max="200"
            ...
            {result && (
              <div className="bg-gray-800 rounded-lg p-6 space-y-4">
                <h3 className="text-xl font-bold text-primary">
                  Verification Result - {result.gameType}
                </h3>
                ...
                <div>
                  ...
                  <div className="font-mono text-sm">
                    ...
                    {result.floats.length > 5 && (
                      <div className="text-gray-500">
                        ... and {result.floats.length - 5} more
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-gray-900 p-4 rounded space-y-3">
                  ...
                  {result.result.minePositions && (
                    <div>
                      <div className="text-sm text-gray-400 mb-2">
                        Mine Positions (Grid {result.result.gridSize})
                      </div>
                      <div
                  ...
                  {result.result.path && (
                    <div>
                      <div className="text-sm text-gray-400">
                        Plinko Path → Final Slot
                      </div>
                      <div className="font-mono text-lg">
                        {result.result.path
                          .map((d: number) => (d === 0 ? "L" : "R"))
                          .join("")}{" "}
                        →{" "}
                        <span className="text-secondary font-bold">
                          Slot {result.result.finalSlot}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  {result.result.result &&
                    typeof result.result.result === "string" && (
                      <div>
                        ...
                        <div className="font-mono text-3xl font-bold text-secondary capitalize">
                          {result.result.result}
                        </div>
                      </div>
                    )}
                  ...
                  {result.result.dangerPositions &&
                    result.gameType === "TOWER" && (
                      <div>
                        <div className="text-sm text-gray-400 mb-2">
                          Tower Danger Positions ({result.result.floors} floors)
                        </div>
                        ...
                        <div className="text-xs text-gray-400 mt-2">
                          Danger positions: [
                          {result.result.dangerPositions.join(", ")}]
                        </div>
                      </div>
                    )}
                  ...
                  {/* STAIRS */}
                  {result.result.dangerPositions &&
                    result.gameType === "STAIRS" && (
                      <div>
                        <div className="text-sm text-gray-400 mb-2">
                          Stairs Danger Positions ({result.result.steps} steps)
                        </div>
                  ...
                  {/* HILO */}
                  {result.result.currentCard !== undefined &&
                    result.result.nextCard !== undefined && (
                      <div>
                        <div className="text-sm text-gray-400">HiLo Cards</div>
                        <div className="font-mono text-2xl font-bold text-secondary">
                          Current: {result.result.currentCard} → Next:{" "}
                          {result.result.nextCard}
                        </div>
                      </div>
                    )}
                ...
                <div className="text-xs text-gray-500 space-y-1">
                  <p>
                    <strong>Stake's Provably Fair Algorithm:</strong>
                  </p>
                  <p>1. currentRound = floor(cursor / 32)</p>
                  <p>
                    2. HMAC-SHA256(serverSeed, "clientSeed:nonce:currentRound")
                  </p>
                  <p>3. Bytes → floats (4 bytes per float)</p>
                  <p>4. Floats mapped to game-specific outcomes</p>
                  <p>
                    5. Fisher-Yates shuffle for games requiring unique outcomes
                  </p>
                </div>
              </div>
            )}
